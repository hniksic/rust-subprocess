...
fn communicate_input() {
    ...
    .unwrap();
    let (out, err) = p.communicate("hello world").read().unwrap();
    assert!(out.is_empty());
    assert!(err.is_empty());
...
fn communicate_input_output_long() {
    ...
    let (out, err) = p.communicate(&input).read().unwrap();
    assert_eq!(&out[..], &input[..]);
...
fn communicate_size_limit_small() {
    ...
    .unwrap();
    let mut comm = p.communicate([]).limit_size(2);
...
fn communicate_size_limit_large() {
    ...
    .unwrap();
    let mut comm = p.communicate([]).limit_size(10_000);
...
fn communicate_stdin_only() {
    ...
    .unwrap();
    let (out, err) = p.communicate("test data").read().unwrap();
    assert!(out.is_empty());
    assert!(err.is_empty());
    assert!(p.wait().unwrap().success());
}
...
fn communicate_size_limit_zero() {
    ...
    .unwrap();
    let mut comm = p.communicate([]).limit_size(0);
    let (out, err) = comm.read().unwrap();
    assert!(out.is_empty());
    assert!(err.is_empty());
...
fn communicate_size_limit_stderr() {
    ...
    .unwrap();
    let mut comm = p.communicate([]).limit_size(4);
    let (out, err) = comm.read().unwrap();
...
fn communicate_timeout_zero() {
    ...
    .unwrap();
    let result = p.communicate([]).limit_time(Duration::from_secs(0)).read();
...
fn communicate_large_bidirectional() {
    ...
    let (out, _) = p.communicate(&input).read().unwrap();
    assert_eq!(out, input);
    assert!(p.wait().unwrap().success());
}
...
fn communicate_partial_read_continue() {
    ...
    let mut comm = p.communicate([]).limit_size(3);
...
fn communicate_very_long_lines() {
    ...
    .unwrap();
    let (out, _) = p.communicate([]).read().unwrap();
...
fn communicate_timeout_with_partial_and_continue() {
    ...
    let mut comm = p.communicate([]).limit_time(Duration::from_millis(100));
    let mut out = vec![];
    let mut err = vec![];
